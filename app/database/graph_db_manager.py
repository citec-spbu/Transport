...
class GraphDBManager(ABC):
    def __init__(self, analysis_context: AnalysisContext):
        ...
        self.db_graph_parameters = analysis_context.neo4j_DB_graph_parameters

    def enrich_db_parameters(self, analysis_context: AnalysisContext):
        analysis_context.neo4j_DB_graph_parameters.city_name = self.city_name
        analysis_context.neo4j_DB_graph_parameters.node_geometry_identity = self.node_geometry_identity()
        analysis_context.neo4j_DB_graph_parameters.main_rels_name = self.get_main_rels_name()
        analysis_context.neo4j_DB_graph_parameters.main_node_name = self.get_main_node_name()
        analysis_context.neo4j_DB_graph_parameters.weight = self.get_weight()
...
class OneTypeNodeDBManager(GraphDBManager):
    ...
    def update_db(self):
        ...
        if nodes is None and relationships is None:
            print(f"Graph for {self.city_name} is empty!")
            return
    ...
    def get_main_node_name(self) -> str:
        return self.db_graph_parameters.main_node_name.replace('-', '')

    def get_main_rels_name(self) -> str:
        return self.db_graph_parameters.main_rels_name.replace('-', '')
